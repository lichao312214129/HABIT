"""
Other designs from atomic components
====================================

**Background.** :doc:`/auto_examples/01_complete/plot_04_atomic_two_step`
...
:doc:`/auto_examples/01_complete/plot_02_inside_each_subject` fits one
model per patient on that patient's voxels, and the pooled-voxel design
of :doc:`/auto_examples/01_complete/plot_03_pooled_voxels` fits one
...
DCE phases, ROI ``LAP``, subject-level winsorize (5 % / 5 %) + min-max,
...
one_step_result = Study(one_step_spec).fit_predict(train)
...
# The fitted chain is APPLIED (not refitted) to each patient's rows.
binned_units = [
...
pooled_model = make_fitter().fit(binned_units, cohort=train)
# The centroids live in the binned space, so the bin edges travel with
# the model; a new patient must be binned with these same edges.
pooled_model = pooled_model.with_cohort_preprocessing(cohort_chain.state, cohort_chain.spec.to_dict())
print(pooled_model.summary())
assigner = pooled_model.assigner("nearest_centroid")
pooled_maps = [assigner(one) for one in binned_units]
...
pooled_result = Study(pooled_spec).fit_predict(train)
...
#   :doc:`/auto_examples/01_complete/plot_02_inside_each_subject` and
...
# * Each component with its options: :doc:`/auto_examples/02b_stages/index`.
