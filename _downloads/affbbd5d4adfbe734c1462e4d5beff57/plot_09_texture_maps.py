...
texture). Subject-level winsorize + min-max, 30 k-means supervoxels,
...
TEXTURE = {
    ...
    "kernel_radius": 1,
    # PyRadiomics-style settings: grey-level bin width and the GLCM
    # features to compute per voxel.
    "params": {
        "setting": {"binWidth": 25},
        "featureClass": {"glcm": ["Contrast", "Idm"]},
    },
}
...
result_texture = Study(spec_texture).fit_predict(train)
result_both = Study(spec_both).fit_predict(train)
...
# How many habitats
# -----------------
# The elbow curve of each run. The marked point is the count kept.
for label, model in (("texture only", model_texture), ("intensity + texture", model_both)):
    fig = plot_cluster_validation_from_report(model.preprocessing_state["selection_report"])
    fig.suptitle(f"Elbow: {label}")
...
#   :doc:`/auto_examples/02b_stages/plot_04_voxel_texture` and
#   :doc:`/auto_examples/02b_stages/plot_05_voxel_texture_gpu`.
# * Clustering a texture field directly on one subject:
#   :doc:`/auto_examples/02b_stages/plot_07_texture_habitats`.
# * How the subject-level and cohort-level preprocessing stages behave:
#   :doc:`/auto_examples/02b_stages/plot_08_preprocess_features`.
# * The same design on intensity alone:
#   :doc:`/auto_examples/01_complete/plot_08_single_sequence`.
