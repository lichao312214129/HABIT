...
:doc:`/auto_examples/01_complete/plot_01_two_step_spec` twice: fit A on
...
result_a = Study(spec).fit_predict(cohort_a)
result_b = Study(spec).fit_predict(cohort_b)
...
#
# The subject-normalized intensities (winsorize + min-max with each
...
# :doc:`/auto_examples/01_complete/plot_02_inside_each_subject`). On
...
#   Dice): :doc:`/auto_examples/01_complete/plot_15_segmentation_robustness`.
# * Train, save and label new patients with one model (no matching
#   needed): :doc:`/auto_examples/01_complete/plot_18_train_save_predict`.
