...
two-step design, pick the number of habitats with the elbow rule, view
the habitat maps, get a one-row-per-patient feature table (volume
...
subject-level winsorize (5 % / 5 %) then min-max; 30 k-means
supervoxels per tumour; pool; cohort-level 10-bin uniform binning;
...
* **subject-level normalization** -- each patient's voxel features are
  ...
  5 % at each end, then min-max to 0..1). MR intensities are arbitrary
...
# (:doc:`/auto_examples/01_complete/plot_02_inside_each_subject`). Drop
# only ``partition`` and voxels of all patients are clustered together
# (:doc:`/auto_examples/01_complete/plot_03_pooled_voxels`).
spec = HabitatSpec(
    name="two_step_spec",
    stages=(
        ...
        # own 5th..95th percentile, so a few very bright or very dark
        ...
        # patient's own min and max. A globally darker scan now uses the
        # same 0..1 range as a brighter one. Trade-off: the absolute
        # whole-tumour enhancement level is removed; habitats describe
        # the pattern inside each tumour, not how bright it is overall.
        ...
        # preprocess_cohort (cohort-level): 10 equal-width bins per feature,
        # with bin edges learned on the pooled training supervoxels only.
        # The edges are stored in the model and reused for new patients.
...
# How many habitats
# -----------------
# The fitter scores every candidate count (here the k-means inertia,
# which always falls as K grows) and keeps the elbow, where adding one
# more habitat stops paying off. The report is stored on the model, so
# the choice can be audited later from the saved file.
...
# ----------------------
# * The elbow rule kept **K = 4** habitats on subj001 + subj002.
...
# %%
# Is the new patient one big habitat?
# -----------------------------------
...
#   ``Study``: :doc:`/auto_examples/01_complete/plot_04_atomic_two_step`.
