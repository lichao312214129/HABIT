...
def build_spec(name: str, normalize: bool) -> HabitatSpec:
    ...
    Args:
        ...
        normalize: Keep the subject-level winsorize + min-max stages.
    ...
    if normalize:
        stages += [
            # subject-level: clip to this patient's own 5th..95th percentile.
...
poor_result = Study(poor_spec).fit_predict(train)
...
# poor definition without subject-level winsorize / min-max: K = 5):
#
...
#   :doc:`/auto_examples/01_complete/plot_13_cleaning_maps`.
...
#   :doc:`/auto_examples/02c_metrics/plot_01_volume_fractions` and
