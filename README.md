...
- * 逐个调试讲课需要用到的图像预处理方法、特征提取方法FIXED、聚类方法FIXED、机器学习方法
- * 用纯python实现逐步回归特征筛选. FIXED
- 聚类的时候能否保存聚类模型，以便下次使用？
- ** FIXED 解决了habitat_analysis.py中特征提取构造错误问题。以前create时只考虑了image,所以会导致这个情况出错：concat(raw(t1), local_entropy(t1))。因为image只有t1，所以最后只有一个特征。现在key改为method-image形式。