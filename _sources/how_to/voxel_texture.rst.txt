...
Goal: compute and display **voxel-level texture** maps — densified
``voxel_radiomics`` columns (here GLCM Contrast) — as publication 2D slices.
...
For multi-GPU cohort timings, see :doc:`/auto_examples/06_parallel_runs/plot_03_several_gpus`. Scheduling (serial, skip, resume, process pool) is :doc:`/auto_examples/06_parallel_runs/index`. This page keeps
...
**Install in the same** ``habit`` **env.** IBSI ``voxel_radiomics`` (all
three rows above) needs **PyRadiomics** (:doc:`/tutorial/installation` step 6).
The two GPU
...
``feature=0`` is the first column (GLCM Contrast). A column name works too.
Default ``mode="overlay"`` paints **opaque** feature colours inside
...
.. figure:: ../_static/images/examples/voxel_texture_overlay.png
   :alt: Opaque GLCM Contrast overlay on anatomy
   :width: 520
