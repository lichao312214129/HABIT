...
#
"""
Manual example: two-step habitat analysis via ``habit.recipes.two_step``.
...
# --- load cohort from conventional directory layout ---
...
# --- run recipe and save ---
result = two_step(cohort, spec)
OUT_DIR.mkdir(parents=True, exist_ok=True)
result.save(OUT_DIR)

print(f"Saved {len(result.habitat_maps)} habitat maps to: {OUT_DIR}")
if result.habitat_model is not None:
    print(f"Cohort-level habitat count: {result.habitat_model.n_habitats}")
if result.features is not None:
    print(f"Feature table shape: {result.features.frame.shape}")
print("Done.")
