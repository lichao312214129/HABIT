...
#
"""
Manual example: stratified K-fold cross-validation via ``habit.recipes.cross_validate``.

Pure Python API — MLSpec built in code, FeatureTable loaded from CSV.

Run from repository root (py310)::

    E:\\conda\\mconda\\envs\\py310\\python.exe tests/examples/manual_ml_cross_validate.py
"""
...
spec = MLSpec(
    ...
    feature_selectors=(
        ...
        Spec(name="variance", params={"top_k": 20, "plot_variances": True}),
    ),
    random_seed=SEED,
)
...
result = cross_validate(table, spec, n_splits=N_SPLITS, seed=SEED)
...
print(f"Saved CV summary: {metrics_path}")
print("Done.")
