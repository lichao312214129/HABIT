...
#
"""Table-format reliability statistics.

This L3 module computes ICC, Cohen's Kappa, Fleiss' Kappa, and
Krippendorff's Alpha from already loaded :class:`pandas.DataFrame` objects.
It deliberately does not read or write files.  In particular, it is distinct
from :mod:`habit.kernels.voxel_icc`: that L0 module estimates voxel-level
precision and its confidence intervals, while this module preserves the
historical table-analysis JSON statistics, including Pingouin's p-values and
...
def analyze_feature_tables(
    ...
    """
    Analyze reliability metrics for aligned features across measurement tables.

    Args:
        data_frames: One already-loaded feature table per measurement session.
            The index is the target identifier used to align sessions.
        file_names: One stable session name per table, used in the group key
            and as the rater column in Pingouin's long-format input.
        metrics: Historical metric names accepted by :func:`create_metric`.
        selected_features: Optional subset of common feature columns.

    Returns:
        The historical nested result payload:
        ``{session_a_vs_session_b: {feature: {metric: result}}}``.

    Raises:
        ValueError: If ``data_frames`` and ``file_names`` do not have the
            same length. Other per-feature failures retain the historical
            fail-soft result entries.
    """
    metrics = metrics or ['icc2', 'icc3']
    if len(data_frames) != len(file_names):
        raise ValueError(
            "analyze_feature_tables requires one file name per data frame; "
            f"got {len(data_frames)} data frame(s) and {len(file_names)} name(s)."
        )
