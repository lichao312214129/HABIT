...
- the label-based metrics (``accuracy`` ... ``f1_score``) are computed from
  ...
  now in plain NumPy via :func:`habit.evaluation._base.confusion_matrix`;
...
from habit.evaluation._base import binary_class_scores, confusion_matrix
...
class HosmerLemeshowPValueMetric(_SpecMixin):
    ...
    def __init__(self, n_groups: int = 10) -> None:
        """Initialize the decile-style calibration test with positive group count."""
        if isinstance(n_groups, bool) or not isinstance(n_groups, int) or n_groups < 2:
            raise ValueError(f"n_groups must be an integer >= 2; got {n_groups!r}.")
