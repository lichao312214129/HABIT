...
real Interface live across several step implementations.  ``SubjectHabitatState``
...
@dataclass
class SubjectHabitatState:
    """
    Per-subject state passed between individual-level habitat pipeline steps.
    ...
    @classmethod
    def empty(cls) -> "SubjectHabitatState":
        """Return the empty state used at the start of a subject pipeline."""
        return cls()
    ...
    def _require(field_name: str, value: Any, step_name: str) -> Any:
        if value is None:
            raise ValueError(
                f"{step_name} requires '{field_name}' in SubjectHabitatState. "
