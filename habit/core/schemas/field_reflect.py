...
Inspects Pydantic model fields and produces structured descriptors that GUI
form builders can render without hardcoded field names.

Key design:
- Schema is the single source of truth; adding a Field() in a Pydantic model
  automatically makes it appear in generated forms.
...
# Fields that are internal and should not be rendered in user forms.
...
def _extract_ui_metadata(field_info: FieldInfo) -> dict:
    ...
    if isinstance(extra, dict):
        ui_keys = ("group", "order", "widget", "visible_if", "label", "help_text", "choices")
...
def reflect_field(name: str, field_info: FieldInfo) -> FieldDescriptor:
    ...
    is_list = list_origin is list or list_origin is tuple
    ...
    if choices is None and ui_meta.get("choices"):
        choices = list(ui_meta["choices"])
        kind = "literal"
...
def group_descriptors(
    ...
    groups: Dict[str, List[tuple[int, FieldDescriptor]]] = {}
...
def nested_descriptors(desc: FieldDescriptor) -> List[FieldDescriptor]:
