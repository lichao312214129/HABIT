...
from habit.domain.image_preprocessing._subject import (
    mask_array,
    rebuild_subject,
    replace_from_sitk,
    select_modalities,
...
from habit.schemas.steps.preprocessing import (
    AdaptiveHistogramEqualizationParams,
    HistogramStandardizationParams,
    N4CorrectionParams,
    RegistrationParams,
    ReorientationParams,
    ResampleParams,
    ZScoreNormalizationParams,
)
...
class N4Correction:
    ...
    def __call__(
        ...
        *,
        ...
        """Correct each selected modality; optional ROI restricts the fit."""
        sitk_mask = (
            subject.mask(mask_roi).to_sitk()
            if mask_roi is not None and mask_roi in subject.masks
            else None
        )
        new_images, new_masks = _copy_maps(subject)
        for modality in select_modalities(subject, images):
...
class Registration:
    ...
    def __call__(
        ...
        *,
        ...
        """Register moving modalities onto ``fixed_image``."""
        from habit.utils.image_registration_apply import apply_registration_to_subject

        return apply_registration_to_subject(
            subject,
            images=select_modalities(subject, images),
            mask_roi=mask_roi or (self.mask_key or None),
            fixed_image=self.fixed_image,
            backend=self.backend,
            type_of_transform=self.type_of_transform,
            metric=self.metric,
            ...
            use_mask=self.use_mask,
            replace_by_fixed_image_mask=self.replace_by_fixed_image_mask,
            elastix_parameter_files=self.elastix_parameter_files,
            elastix_path=self.elastix_path,
            transformix_path=self.transformix_path,
            elastix_threads=self.elastix_threads,
            elastix_parameter_overrides=self.elastix_parameter_overrides,
            sitk_params={
                "number_of_histogram_bins": self.number_of_histogram_bins,
                "metric_sampling_percentage": self.metric_sampling_percentage,
                "shrink_factors_per_level": self.shrink_factors_per_level,
                "smoothing_sigmas_per_level": self.smoothing_sigmas_per_level,
                "learning_rate": self.learning_rate,
                "number_of_iterations": self.number_of_iterations,
                "bspline_mesh_size": self.bspline_mesh_size,
                "bspline_order": self.bspline_order,
            },
        )


PreprocessorRegistry.register_params_model(
    "zscore_normalization", ZScoreNormalizationParams
)
PreprocessorRegistry.register_params_model("resample", ResampleParams)
PreprocessorRegistry.register_params_model("reorientation", ReorientationParams)
PreprocessorRegistry.register_params_model("n4_correction", N4CorrectionParams)
PreprocessorRegistry.register_params_model(
...
PreprocessorRegistry.register_params_model("registration", RegistrationParams)
