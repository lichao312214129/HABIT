...
#
"""Shared machinery for the built-in regressors."""
...
class SklearnRegressorBase:
    ...
    the same guard the classifier base provides, since schema drift is
    ...
    def fit(self, table: FeatureTable) -> "SklearnRegressorBase":
        """Train on a table with a continuous outcome column."""
        y = outcome_series(table, owner=f"regressor.{self._spec_name}")
    ...
    def _matrix_for_predict(self, table: FeatureTable) -> pd.DataFrame:
        ...
        if self._estimator is None:
            raise HABITAPIError(
                f"regressor.{self._spec_name} must be fitted before predict."
            )
        missing = [
        ...
        if missing:
            raise HABITAPIError(
                f"regressor.{self._spec_name} was fitted on feature columns "
    ...
    def predict(self, table: FeatureTable) -> pd.Series:
        """Predict the continuous response indexed by the identifier columns."""
        X = self._matrix_for_predict(table)
        values = np.asarray(self._estimator.predict(X), dtype=np.float64)
        return pd.Series(
            values,
            index=X.index,
