...
class CoxPhSurvival(_SpecParamsMixin, SurvivalModelBase):
    ...
    def __init__(self, penalizer: float = 0.0, l1_ratio: float = 0.0) -> None:
        """Initialize the Cox model with non-negative penalty controls."""
        super().__init__()
        if penalizer < 0:
            raise ValueError(f"penalizer must be non-negative; got {penalizer!r}.")
        if not 0 <= l1_ratio <= 1:
...
class RandomSurvivalForest(_SpecParamsMixin, SurvivalModelBase):
    ...
    def __init__(
        ...
        """Initialize the forest with positive tree-count and sample controls."""
        super().__init__()
        for name, value in (
...
class GradientBoostingSurvival(_SpecParamsMixin, SurvivalModelBase):
    ...
    def __init__(
        self,
        loss: Literal["coxph"] = "coxph",
        learning_rate: float = 0.1,
        n_estimators: int = 100,
        max_depth: int = 3,
        subsample: float = 1.0,
    ) -> None:
        """Initialize Cox-loss boosting with valid positive hyperparameters."""
        super().__init__()
        if loss != "coxph":
            raise ValueError(f"loss must be 'coxph'; got {loss!r}.")
        for name, value in (
