...
* :class:`TableTransformerEstimator` wraps a
  :class:`~habit._table_protocols.TablePreprocessor` or
  :class:`~habit._table_protocols.FeatureSelector`.
...
:class:`~habit.pipeline.TablePipeline` -- itself an
``sklearn.pipeline.Pipeline`` subclass -- is built out of them, and
``habit.compat`` is a frozen legacy surface that must not grow new
capability.

They cannot live in ``habit.adapters`` (L1) either: they depend on
``habit._table_protocols`` (L3), and the layering forbids an upward
import.
...
Typical use::
    ...
    from habit.pipeline.sklearn_interop import as_classifier, as_transformer
...
class TableTransformerEstimator(TransformerMixin, BaseEstimator):
    ...
    Args:
        component: A :class:`~habit._table_protocols.TablePreprocessor`
            or :class:`~habit._table_protocols.FeatureSelector`
            implementation.
        copy_on_fit: When ``True`` (the default, and the behaviour every
            ...
            :class:`~habit.pipeline.TablePipeline` needs: its
    ...
    def _log_before(self, table: FeatureTable) -> None:
        ...
        # Pre-format mappings before handing them to ``logging``. Python's
        # ``LogRecord`` treats a single mapping argument specially, which can
        # make capture handlers interpret component parameters as formatting
        # substitutions rather than a value to display.
        _logger.info(f"  Parameters: {dict(spec.params)}")
...
class TableClassifierEstimator(ClassifierMixin, _TableModelEstimatorBase):
    """
    Adapt a HABIT :class:`~habit._table_protocols.Classifier` to sklearn.
...
class TableRegressorEstimator(RegressorMixin, _TableModelEstimatorBase):
    """
    Adapt a HABIT :class:`~habit._table_protocols.Regressor` to sklearn.
...
class TableSurvivalEstimator(_TableModelEstimatorBase):
    """
    Adapt a HABIT :class:`~habit._table_protocols.SurvivalModel`.
