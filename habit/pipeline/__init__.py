...
from habit.pipeline.outcome_access import outcome_series, survival_target
from habit.habitat_model.postprocess import ConnectedComponentPostprocess
from habit._protocols import (
...
from habit.pipeline.sklearn_interop import (
    FrameToTable,
    as_outcome_model,
    as_transformer,
...
from habit._table_protocols import (
    Classifier,
    FeatureSelector,
    Metric,
    RegressionMetric,
...
def voxel_units(field: VoxelFeatureField) -> Supervoxelization:
    ...
    block one-step designs outside a ``SubjectPipeline``.
...
#: Version 3 records v2 capability-package implementation paths. Earlier
#: archives pickle removed ``habit.domain`` classes and must be rejected before
#: unpickling, otherwise migration failures would be opaque.
...
def _build_sklearn_steps(
    ...
    Layout, always: a :class:`~habit.pipeline.sklearn_interop.FrameToTable` head
    (so an sklearn cross-validation driver can hand the pipeline a plain,
    row-sliceable frame), then one
    :class:`~habit.pipeline.sklearn_interop.TableTransformerEstimator` per
...
class TablePipeline(SkPipeline):
    ...
    * The step list always begins with a
      :class:`~habit.pipeline.sklearn_interop.FrameToTable` head named
    ...
    def set_random_state(self, seed: int) -> None:
        ...
        implementing :class:`~habit._protocols.Seedable`; deterministic
    ...
    def load(cls, path: Union[str, Path]) -> "TablePipeline":
        ...
        Version 3 is the first v2 archive format. Versions 1 and 2 pickle
        v1 ``habit.domain`` classes, so they are explicitly rejected before
        unpickling with migration guidance. A file whose ``format_version``
        this build does not know is never guessed at: silently loading a
        wrong-but-plausible pipeline would produce numbers nobody could trace.
        ...
        with zipfile.ZipFile(source, "r") as archive:
            ...
            if file_version < _PIPELINE_FORMAT_VERSION:
                raise CompatibilityError(
                    f"{source} is a HABIT v1 .habitpipeline archive "
                    f"(format version {file_version}). HABIT v2 does not "
                    "load pickled habit.domain components. Recreate and "
                    "save the pipeline with HABIT v2, or use HABIT v1 to "
                    "run the legacy artifact."
                )
            if file_version > _PIPELINE_FORMAT_VERSION:
                raise CompatibilityError(
