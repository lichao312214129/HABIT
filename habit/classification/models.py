...
:class:`~habit._protocols.Seedable` contract (v1.0 naming decisions).
...
from habit.classification._base import SklearnClassifierBase
...
class _SpecParamsMixin:
    ...
    def __getattr__(self, name: str) -> Any:
        """Expose every declared constructor parameter as a sklearn-style attribute."""
...
class GaussianNbClassifier(_SpecParamsMixin, SklearnClassifierBase):
    ...
    def __init__(
        ...
        """Initialize Gaussian NB with a strictly positive variance smoother."""
        super().__init__()
        if not isinstance(var_smoothing, (int, float)) or var_smoothing <= 0:
            raise ValueError(
                f"var_smoothing must be positive; got {var_smoothing!r}."
            )
        self.priors = priors
