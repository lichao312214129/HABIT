...
A universal module for resolving relative paths in configuration files.
All relative paths are resolved relative to the configuration file's directory.

This module provides:
    - PathResolver: A class for flexible path resolution
    - resolve_config_paths: A convenience function for quick path resolution

Design Principles:
    - Elegant: Clean, readable code with clear separation of concerns
    - Concise: Minimal code, maximum functionality
    - Extensible: Easy to add new path patterns or custom resolution logic
    - Universal: Works with any YAML/JSON configuration structure

Path Detection Strategy:
    1. Key-based detection: Field names ending with _path, _dir, _file, etc.
    2. Value-based detection: Values starting with ./, .\, ../, ..\
    3. Extension-based detection: Values ending with common file extensions
    4. Path separator detection: Values containing / or \ with path-like patterns

Example:
    >>> from habit.utils.path_resolver import resolve_config_paths
    >>> config = resolve_config_paths(config_dict, config_path)
    
    >>> # Or with custom path patterns
    >>> from habit.utils.path_resolver import PathResolver
    >>> resolver = PathResolver(config_path, extra_patterns=['my_custom_path'])
    >>> config = resolver.resolve(config_dict)
