...
from habit.supervoxel._base import partition_from_voxel_labels
...
class KMeansSupervoxelizer:
    ...
    def __init__(
        ...
        if any(
            ...
            for value in (n_supervoxels, max_iter, n_init)
        ):
            raise HABITAPIError(
                "n_supervoxels, max_iter, and n_init must be positive integers."
            )
...
class GmmSupervoxelizer:
    ...
    def __init__(
        ...
        if covariance_type not in {"full", "tied", "diag", "spherical"}:
            raise HABITAPIError(
                f"Unsupported covariance_type {covariance_type!r}; expected "
