...
def extract_voxel_texture(
    ...
    *,
    ...
    :func:`~habit.precision.precision_panel`.

    Args:
        image: Intensity volume.
        mask: ROI mask; one row is emitted per foreground voxel.
        kernel_radius: Neighbourhood radius in voxels (the paper's ``R``).
            Must be a positive int. Radius 1 is 3x3x3; radius 3 is 7x7x7.
            Default 3.
        bin_width: Grey-level discretisation width (the paper's ``B``).
            Must be a positive float. Typical CT pair is 12 vs 25 HU.
            Default 12.0.
        feature_classes: Optional PyRadiomics class-to-names map, e.g.
            ``{"firstorder": ["Entropy", "Mean"], "glcm": ["Contrast"]}``.
            Default None (bundled voxel preset, unless ``params`` is set).
        params: Full in-memory PyRadiomics settings. Default None.
            Mutually exclusive with ``feature_classes``. ``bin_width``
            still overwrites ``setting.binWidth``.
        voxel_batch: ROI voxels per batch. Allowed: positive int or
            ``"auto"`` (size from VRAM). Default 1000.
        use_torch_radiomics: Allowed ``"auto"``, ``True``, or ``False``.
            Default ``"auto"``.
        use_gpu_matrices: Allowed ``"auto"``, ``True``, or ``False``.
            Default ``"auto"``. GPU matrices stay bit-identical to CPU.
