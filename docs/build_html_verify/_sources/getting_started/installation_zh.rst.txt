...
``requirements.txt`` 已包含 **CPU 版** PyTorch（``torch==2.4.0+cpu``，阿里云镜像）。**有 NVIDIA GPU 且需加速** 时，请先执行 ``pip install -r requirements-gpu.txt --upgrade --force-reinstall``，再执行 ``pip install -e .`` （见下文）。
