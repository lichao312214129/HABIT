...
.. list-table::
   ...
   * - ``managers/clustering_service.py``
     - 聚类算法创建、最佳聚类数选择、训练状态保存。
   * - ``managers/result_writer.py``
...
2. ``HabitatAnalysis._inject_services_into_pipeline`` 用
   ``_PIPELINE_SERVICE_ATTRS`` 白名单注入当前运行时 manager。
3. ``FeatureService`` 只更新当前数据路径和日志目标，保留已训练状态。
...
* 如需新的输出后处理，集中放在 ``HabitatAnalysis`` 或 ``ResultWriter``，
  不要在 CLI 层分叉。
...
* 旧 ``strategies/`` 子包和 ``pipeline_builder.py`` 已删除；不要按旧三层结构
  新增代码。
* manager 注入用显式白名单 ``_PIPELINE_SERVICE_ATTRS``；新增 manager 必须主动
  修改白名单和注入逻辑。
