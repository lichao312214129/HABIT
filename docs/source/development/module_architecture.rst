...
.. list-table::
   ...
   * - ``services/habitat_image_writer.py``
     - 将 supervoxel/habitat 等 **NRRD 标签图** 落盘，并与 ``mask_info_cache`` 协作；不写 ``habitats.csv``（CSV 由 ``result_publisher`` 负责）。
   * - ``services/result_publisher.py``
     - ``HabitatResultPublisher``：在 ``fit``/``predict`` 末尾按约定列顺序写出 ``habitats.csv``，需要时委托 ``HabitatImageWriter`` 补充标签图。
   * - ``clustering/``
   ...
   * - ``feature_preprocessing/``
     - 生境**特征**预处理（与 ``habit.core.preprocessing`` 图像预处理不同）：
       统一 DataFrame 进/出 pipeline（``pipeline.apply_preprocessing_pipeline``）；
       内置方法通过 ``@register_preprocessing`` 注册到
       ``PreprocessingMethodFactory``；群体级状态由 ``PreprocessingState``
       （``baseline`` + 每步 ``step_states``）维护。
   * - ``habitat_features/``
...
``config.HabitatSegmentation.clustering_mode`` 选择 recipe：

* ``two_step``：voxel → supervoxel（个体级）→ 群体级 habitat（跨 subject 聚类）。
...
* ``GroupLevelStep``：所有 subject 汇总后顺序执行；包含训练阶段
  *有状态* 的 ``GroupPreprocessingStep`` 与 ``GroupClusteringStep``。
...
下表给出各步骤的标记类、载荷类型、状态语义。个体级步骤在实现中以 ``HabitatSubjectData``
为每个 subject 的载荷（外层仍表现为 ``Dict[subject_id, HabitatSubjectData]``）。
表列名 ``Subject``、``Supervoxel``、``Habitats``、``Count`` 在 ``ResultColumns`` 集中定义。
...
.. list-table::
   ...
   * - ``VoxelFeatureExtractor``
     ...
     - Stateless
     - 全部
     - ``Dict[sid, HabitatSubjectData]``（空 → 填入 ``features`` / ``raw`` / ``mask_info``）
   * - ``IndividualPreprocessingStep``
     - Individual
     - Stateless
     - 全部
     - 同上 → 同上（features 被清洗与 individual 级预处理）
   ...
   * - ``MergeSupervoxelFeaturesStep``
     ...
     - Stateless
     - two_step / one_step
     - 上一步输出 → ``Dict[sid, HabitatSubjectData]``（写入 ``supervoxel_df`` 等）。在 ``mean_voxel_features``
   ...
   * - ``CombineSupervoxelsStep``
     ...
     - Stateless
     - two_step / one_step
     - ``Dict[sid, HabitatSubjectData]`` → 单一 ``pd.DataFrame``
   * - ``ConcatenateVoxelsStep``
     - Group
     - Stateless
     - direct_pooling
     - ``Dict[sid, HabitatSubjectData]`` → 单一 ``pd.DataFrame``（带 Subject 列）
...
``PreprocessingState``，``GroupClusteringStep`` 自带 ``clustering_model``
...
.. code-block:: text
   ...
   images + masks  (Dict[subject_id, {}])
     ...
     -> IndividualPreprocessingStep            # individual, stateless
     ...
     -> GroupClusteringStep               # group, STATEFUL
...
direct_pooling 在 ``IndividualPreprocessing`` 之后直接 ``ConcatenateVoxels``
→ 可选 ``GroupPreprocessing`` → ``GroupClustering``。
...
* ``mask_info`` 在 worker 内被收集到 pipeline 主进程的 ``mask_info_cache``，
  供 ``HabitatResultPublisher`` / ``HabitatImageWriter`` 在流水线结束后落盘标签图时复用。
...
* 输出列差异（如 one-step 把 ``Supervoxel`` 镜像为 ``Habitats``）放在对应
  step 内；pipeline 之后的 ``habitats.csv`` 与可选标签图发布由
  ``HabitatResultPublisher.publish`` 统一处理。
...
1. 在 ``feature_preprocessing/`` 下实现 ``BaseFeaturePreprocessing`` 子类
   （``fit`` / ``transform``，输入输出均为 feature DataFrame）。
2. 用 ``@register_preprocessing("method_name")`` 注册；若会删列，设
   ``changes_columns = True``。
3. 在 ``config_schemas.PreprocessingMethod.method`` 的 Literal 中追加方法名。
4. 将 ``changes_columns=True`` 的方法名同步到
   ``DROPPING_PREPROCESSING_METHODS``（与 ``two_step`` subject 级校验一致）。
5. 参考模板 ``feature_preprocessing/custom_preprocessing_template.py``。
6. 确保模块在运行时被 import 一次，使注册装饰器生效（可参考
   ``builtin_methods.py`` 的加载方式）。

个体级与群体级共用同一套 handler；差别仅在于群体级通过
``PreprocessingState`` 在训练时缓存 ``baseline`` 与各步 ``step_states``，
预测时复用。
