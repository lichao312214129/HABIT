...
.. list-table::
   ...
   * - ``pipelines/subject_state.py``
     - ``SubjectHabitatState``：个体级各 step 传递的显式状态（字段随流水线逐步填充）。
   * - ``pipelines/steps/*.py``
...
下表给出各步骤的标记类、载荷类型、状态语义。个体级步骤在实现中以 ``SubjectHabitatState``
为每个 subject 的载荷（外层仍表现为 ``Dict[subject_id, SubjectHabitatState]``）。
...
.. list-table::
   ...
   * - ``VoxelFeatureExtractor``
     ...
     - Stateless
     - 全部
     - ``Dict[sid, SubjectHabitatState]``（空 → 填入 ``features`` / ``raw`` / ``mask_info``）
   ...
   * - ``MergeSupervoxelFeaturesStep``
     ...
     - Stateless
     - two_step / one_step
     - 上一步输出 → ``Dict[sid, SubjectHabitatState]``（写入 ``supervoxel_df`` 等）。在 ``mean_voxel_features``
   ...
   * - ``CombineSupervoxelsStep``
     ...
     - Stateless
     - two_step / one_step
     - ``Dict[sid, SubjectHabitatState]`` → 单一 ``pd.DataFrame``
   * - ``ConcatenateVoxelsStep``
     - Group
     - Stateless
     - direct_pooling
     - ``Dict[sid, SubjectHabitatState]`` → 单一 ``pd.DataFrame``（带 Subject 列）
