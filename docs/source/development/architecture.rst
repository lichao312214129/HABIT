...
.. graphviz::
   ...
   digraph architecture {
       ...
       subgraph cluster_l4 {
           ...
           HabitatArch [label="HabitatAnalysis (deep)\n+ HabitatPipeline + services"];
...
* ``habit.core.common`` 不在模块顶层 import 任何业务子包；
  装配由 **各自子包内的域 configurator** 完成（均继承 ``BaseConfigurator`` ，
  基类位于 ``habit.core.common.configurators.base``）；业务类的 import 全部
  **延迟到 factory 方法体内**。
...
* :py:class:`habit.core.habitat_analysis.configurator.HabitatConfigurator`
  ...
  - ``create_habitat_analysis()`` —— 完整装配（注入三个 services + logger）
  - ``create_feature_extractor()`` —— 返回 ``HabitatMapAnalyzer``
...
.. list-table::
   ...
   * - ``habitat_analysis.py`` (**deep**)
     - V1 的 **唯一编排入口**。串联 build、fit/predict、pipeline 持久化；训练/预测末尾通过
       ``HabitatResultPublisher`` 写出 ``habitats.csv`` 并按配置发布标签图。
       内部用 ``_PIPELINE_RECIPES`` 按 ``clustering_mode`` 分发 step 列表；用
       ``_PIPELINE_SERVICE_ATTRS`` 白名单向已加载的 pipeline 注入
       ``FeatureService`` / ``ClusteringService`` / ``HabitatImageWriter``。
   * - ``services/{feature_service,clustering_service,habitat_image_writer,result_publisher}.py``
     - 特征与预处理、聚类、NRRD 标签图落盘；``HabitatResultPublisher`` 负责 CSV 与结果发布（在 ``HabitatAnalysis`` 内构造，不注入各 step）。
       前三个 service 构造时注入 ``HabitatAnalysis`` ，再注入到各 pipeline step。
   * - ``pipelines/base_pipeline.py``
     - sklearn 风格的 ``HabitatPipeline`` + ``BasePipelineStep``。
       ``fit_transform`` / ``transform`` / ``save`` / ``load`` 接口。
   * - ``pipelines/steps/*.py``
     - 多个具体步骤（由 recipe 决定顺序与是否包含可选步；常见全流程约 10 步，见 :doc:`module_architecture`）：
       体素特征、subject 预处理、supervoxel/habitat 个体聚类、supervoxel 级特征合并、跨 subject 拼接、群体预处理与群体聚类等。
   * - ``pipelines/habitat_subject_data.py``
     - ``HabitatSubjectData`` ：个体级 step 之间的显式数据契约。
   * - ``clustering/``
     - K-Means / GMM / SLIC **聚类算法**（与 ``config_schemas.py`` 中 ``algorithm`` 可选值一致；更多算法可通过 ``@register_clustering`` 扩展）
       （注意：此处的 strategy 指算法实现接口：***不是** 已废弃的旧策略模式）。
   * - ``clustering_features/``
     - 体素级 / supervoxel 级聚类前特征抽取实现，由 factory 按配置选择。
   * - ``habitat_features/habitat_analyzer.py``
     - ``HabitatMapAnalyzer`` ：在已生成的生境图上做多进程特征抽取。
   * - ``config_schemas.py``
...
.. list-table::
   ...
   * - ``two_step``
     - voxel features → subject prep → individual clustering (voxel→supervoxel) →
       ...
       merge supervoxel-level features → combine subjects (single DataFrame) →
       optional group prep → group clustering (supervoxel→habitat)
   * - ``one_step``
     - voxel features → subject prep → individual clustering (voxel→habitat) →
       mean voxel features → merge features → combine subjects（无群体聚类步）
   * - ``direct_pooling``
     - voxel features → subject prep → concatenate voxels (cross-subject pooling) → 
       group prep → group clustering
...
.. graphviz::
   ...
   digraph fit_flow {
       ...
       SP [label="IndividualPreprocessing"];
       IC [label="IndividualClustering\nvoxel -> supervoxel"];
       SA [label="Merge +\nCombineSupervoxels"];
       GP [label="GroupPreprocessing"];
       PC [label="GroupClustering\nsupervoxel -> habitat"];
...
2. 用 ``_PIPELINE_SERVICE_ATTRS`` 白名单把当前运行时的三个 service 注入到 step。
...
* **Shallow**：``services`` 包、单个 step、单个 algorithm/extractor。
  这些都是「插件式」的薄壳，遵循固定接口。
...
* **新增 pipeline step**：继承 ``IndividualLevelStep`` 或 ``GroupLevelStep`` ，
  通过构造函数接收 ``feature_service`` / ``clustering_service`` / ``habitat_image_writer``
  中其所需的一个或多个依赖。
...
.. list-table::
   ...
   * - ``workflows/holdout_workflow.py``
     - ``HoldoutWorkflow`` ：训练 + 预测统一执行体（旧名 ``MachineLearningWorkflow`` 已成为 deprecation 子类）。
   * - ``workflows/kfold_workflow.py``
     - ``KFoldWorkflow`` （旧名 ``MachineLearningKFoldWorkflow`` 已成为 deprecation 子类）。
...
用于动态创建特征提取器、聚类算法和机器学习模型。

**实现位置**：

* ``clustering_features/feature_extractor_factory.py`` —— ``FeatureExtractorFactory``
* ``clustering/base_clustering.py`` —— ``ClusteringAlgorithmFactory``
* ``machine_learning/models/factory.py`` —— ``ModelFactory``
...
.. code-block:: python
   ...
   # 使用
   from habit.core.habitat_analysis.clustering import ClusteringAlgorithmFactory

   algo = ClusteringAlgorithmFactory.create_algorithm('kmeans', n_clusters=5)
...
* Logger 注入到各 service / workflow 协作对象
...
1. **消除浅层 Pipeline Steps**
   ...
   **问题**：部分 Step 类只是简单委托给领域 service：
   
   - ``VoxelFeatureExtractor.transform()`` → ``FeatureService.extract_voxel_features()``
   - ``IndividualPreprocessingStep.transform()`` → ``FeatureService`` 侧预处理
   
   **影响文件**：``pipelines/steps/voxel_feature_extraction.py`` 等
   
   **建议**：把协调逻辑提升到 Step，或拆分 ``FeatureService`` 职责。
...
3. **拆分 FeatureService 多职责**
   ...
   **问题**：``FeatureService`` 体积较大：特征提取协调、预处理执行、路径与配置配合等混在一起。
   
   **建议**：拆分为更小、可单独测试的协调器类型（示例：提炼 ``Preprocessor`` /
   ``ConfigResolver`` 等）。
...
5. **增加 Pipeline Steps 测试覆盖**
   ...
   **建议**：为核心 Steps（VoxelFeatureExtractor、IndividualClusteringStep、
   GroupClusteringStep、GroupPreprocessingStep）添加测试。
...
6. **明确 HabitatMapAnalyzer 定位**
   ...
   **问题**：``HabitatMapAnalyzer`` 是导出为公共 API 的后处理特征工具，与生境流水线落盘的
   ``ResultWriter`` 关注点不同但也有数据交接。
   
   **建议**：在文档中写明「训练/预测流水线」vs「仅在已有生境图上提特征」的边界。
...
- Service 注入使用显式白名单 ``_PIPELINE_SERVICE_ATTRS``；新增 collaborator 时必须同步扩展白名单。
- 流水线逐步说明见 :doc:`module_architecture`；模块级说明见
  ``habit/core/habitat_analysis/habitat_analysis.py`` 文件头 docstring。

**machine_learning**：

- 训练与预测共用 ``MLConfig`` 与 ``HoldoutWorkflow`` （旧名 ``MachineLearningWorkflow`` 仍可用），不要恢复独立 ``PredictionWorkflow``。
- 评估/绘图/报告由 ``ReportWriter`` / ``PlotComposer`` / ``ModelStore`` 显式调用，workflow 只做编排。
...
2. 按要改的功能选 domain：
   ...
   - ML 建模：``HoldoutWorkflow`` 、``BaseWorkflow`` 、``PipelineBuilder``
...
* :doc:`module_architecture` —— habitat 等各域逐步 I/O 与状态契约
* ``habit/core/habitat_analysis/habitat_analysis.py`` —— Recipe 派发与 ``fit`` /
  ``predict`` 生命周期说明（文件头注释）
* ``habit/core/habitat_analysis/clustering_features/README.md`` —— 聚类特征抽取扩展
* ``habit/core/habitat_analysis/habitat_features/README.md`` —— 生境后处理特征抽取
