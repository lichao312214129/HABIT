...
.. list-table::
   ...
   * - ``habitat_analysis.py`` (**deep**)
     - V1 的 **唯一编排入口** 。build → fit/predict → 持久化 → 结果后处理。
       ...
       用 ``_PIPELINE_SERVICE_ATTRS`` 显式白名单向已加载的 pipeline 注入
       ``FeatureService`` / ``ClusteringService`` / ``ResultWriter``。
...
* **新增 pipeline step**：继承 ``IndividualLevelStep`` 或 ``GroupLevelStep``，
  通过构造函数接收 ``feature_service`` / ``clustering_service`` / ``result_writer``
  中其所需的一个或多个依赖。
