...
- ``type_of_transform``：**全部可选值**见 :doc:`user_guide/image_preprocessing_zh` 中「registration」小节的列表（与 ``RegistrationPreprocessor`` / ANTs 一致）；常见如 ``Rigid``、``Affine``、``SyN``、``SyNRA``、``SyNOnly``、``SyNCC`` 等；``metric``、``optimizer``；``use_mask``；可选 ``mask_keys``；``replace_by_fixed_image_mask``；其余 ANTs 允许的参数可通过额外键传入（谨慎使用）。
