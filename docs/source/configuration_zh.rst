...
- ``backend``（可选）：``ants``（默认）、``simpleitk``、``elastix``（调用官方 **elastix** / **transformix** 可执行文件；可选 ``elastix_path`` / ``transformix_path``）；详见 :doc:`user_guide/image_preprocessing_zh` 中「registration」。
- ``type_of_transform`` 等对 **ants / simpleitk** 有意义；``elastix`` 后端不使用这些键驱动配准。**全部可选值**（ANTS 路径）见同文档列表；常见如 ``Rigid``、``Affine``、``SyN``、``SyNRA`` 等；``metric``、``optimizer``；``use_mask``；可选 ``mask_keys``；``replace_by_fixed_image_mask``；**``elastix`` 专属**：``elastix_parameter_files``（`LKEB elastix Model Zoo <https://lkeb.ml/modelzoo/>`__ 等提供的 ``.txt``；**不同数据类型与配准任务宜选用不同模板**，说明见 :doc:`user_guide/image_preprocessing_zh` 中 elastix「**参数与数据类型**」）、``elastix_parameter_overrides``（dict，覆盖参数值）、``elastix_path``、``transformix_path``、``elastix_threads``；余下 ANTs 允许的参数可通过额外键传入（谨慎使用）。
...
- ``data_dir`` / ``out_dir``：必选，输入树根与默认输出根；相对路径相对于 **YAML 文件所在目录** 解析。
- ``f``：dcm2niix ``-f`` **原样**，不会被当成相对路径改写（若含 ``/``、``.dcm`` 等与真实路径相仿的写法亦如此）。
- ``filename_format``：弃用别名，语义同 ``f``，二者至少填其一。
- ``dcm2niix_path``：可选；若为相对路径，同样相对于 YAML 目录解析（可省略则使用 ``PATH`` 中的 ``dcm2niix``）。
- ``extra_args``：可选列表，逐项 **原样** 追加到 dcm2niix 命令行（不经「形似路径」自动解析）。
- ``output_dir``：可选；若设置，用作 ``-o`` 目标目录；相对路径相对于 YAML 目录解析；未设置则用 ``out_dir``。
...
- **类型**: 字符串
- **必需**: 当 ``run_mode`` 为 ``predict`` 时 **必填**（指向已保存的 ``*_final_pipeline.pkl``）。
...
- **子参数**:
  ...
  - ``path``: 特征 CSV/Excel 路径（**必需**）。
  - ``name``: 数据集名称（可空字符串，默认 ``""``）。
  - ``subject_id_col``: 受试者 ID 列（**必需**）。
  - ``label_col``: 标签列（**必需**）。
  - ``features``: 仅使用这些列作为特征；省略则使用除 ID/标签等外的数值列（由流水线解析）。
  - ``features_from_log``: 从日志/辅助文件解析特征列名（可选）。
  - ``split_col``: 自定义划分时用到的分组列（可选）。
  - ``pred_col``: 若数据含既有预测列，可供评估对齐（可选）。
...
- ``enabled`` (默认 ``true``): 嵌套 ``VisualizationConfig`` 开关。
...
对应 ``habit.core.habitat_analysis.config_schemas.RadiomicsConfig``，嵌套结构如下。

**paths**（必填）

- ``params_file``: PyRadiomics 参数 YAML（`PyRadiomics 文档 <https://pyradiomics.readthedocs.io/>`__）
