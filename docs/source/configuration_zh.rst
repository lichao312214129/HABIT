...
- **说明**:
  - `train` 与 `predict` 共用同一个 `MLConfig` schema；
    通过 `run_mode` 字段区分（`--mode` 会覆盖 YAML 中的 `run_mode`）。
  - `predict` 模式需要在 YAML 中提供 `pipeline_path`
    （指向已训练的 ``*_final_pipeline.pkl``）。
...
- **执行时机与调用链**:
  ...
  - ``run()`` 中训练模型时，会调用 ``_train_with_optional_sampling(...)``。
