...
除非写明 **无（必填）**，每条参数均列出 **默认值** （由程序在加载 YAML 时自动校验并填入；开发者实现见 ``habit/core/*/config_schemas.py``）。

- YAML 中 **省略该键** → 使用本页所列 **内置默认值** （与仓库模板 YAML 里的示例值可能不同）。
...
HABIT 在 **仓库根目录** 提供 ``config/`` 目录，与 Python 包 ``habit/`` **同级** （示例 YAML 随源码分发；``pip install`` 不会把它们装进 ``site-packages/habit/`` 内部）。
用户可直接参考或复制修改，无需从零编写 YAML：

- **场景索引（建议先读）**：``config/README_CONFIG.md`` （各模块 YAML 与 ``tests/<module>/`` 脚本对照）
- **按 CLI 选文件**：见下文 **「CLI 命令与配置文件对照」** 表
- **按目录浏览**：见下文 **「仓库配置模板索引」**

典型流程：复制例如 ``config/habitat/config_habitat_two_step.yaml`` → 修改 ``#%%====`` 块中的 ``data_dir`` / ``out_dir`` → ``habit get-habitat -c <你的副本.yaml>`` 。

.. note::

   若当前环境仅有已安装的 ``habit`` 包而看不到 ``config/`` 目录，请从 `GitHub 仓库 <https://github.com/lichao312214129/HABIT>`_ 获取完整源码树（ZIP 解压或 ``git clone`` ），与 :doc:`getting_started/installation_zh` 安装说明一致。
...
.. list-table::
   ...
   * - ``habit get-habitat`` — SLIC 超像素
     - 「HabitatSegmentation.supervoxel — SLIC 超像素配置」
   * - ``habit extract``
     - 「特征提取配置参数」
   ...
   * - 内置 ``config/`` 示例 YAML
     - 「内置配置模板（config/）」「CLI 命令与配置文件对照」「仓库配置模板索引」
   * - 仓库模板路径
     - 「仓库配置模板索引」
...
下表汇总 **CLI 命令 → 配置类型 → 仓库内示例 YAML**。部分工具命令 **不需要** YAML，仅使用命令行参数。
...
.. list-table::
   ...
   * - ``habit get-habitat``
     - 生境分析配置
     - ``config/habitat/config_habitat_two_step.yaml`` （Demo；输入 ``demo_data/preprocessed/processed_images``；另有 one_step / direct_pooling 及 ``*_predict.yaml``）
   * - ``habit extract``
     - 特征提取配置
     - ``config/feature_extraction/config_extract_features_demo.yaml`` （Demo）
   * - ``habit model`` / ``habit cv``
     - 机器学习配置
     - ``config/machine_learning/config_machine_learning_radiomics.yaml`` （Demo）；``cv`` 常用 ``config_machine_learning_kfold_demo.yaml``
   * - ``habit compare``
     - 模型对比配置
     - ``config/model_comparison/config_model_comparison_demo.yaml`` （Demo）
   * - ``habit icc``
     - ICC 分析配置
...
**被试/文件列表 YAML** （单独的被试路径表，在 ``data_dir`` 中引用）：

- Demo： ``config/habitat/file_habitat_demo.yaml`` 、``config/preprocessing/files_preprocessing_demo.yaml``
- 开发测试： ``config/preprocessing/files_preprocessing.yaml`` 、``config/habitat/file_habitat.yaml``
...
本节为 **预处理** 配置（``PreprocessingConfig``）。顶层字段 ``Preprocessing`` 的 **键名** 必须与 ``PreprocessorFactory`` 注册名一致；YAML 中书写的子块顺序即执行顺序。
...
本节为 **生境分析** 配置。CLI： ``habit get-habitat -c <yaml>``。Demo 训练见 ``config/habitat/config_habitat_two_step.yaml`` ，预测见 ``config/habitat/config_habitat_two_step_predict.yaml`` ；SLIC 超体素示例见 ``config/habitat/config_habitat_two_step_supervoxel_slic.yaml`` （参数说明见下文 **「HabitatSegmentation.supervoxel — SLIC 超像素配置」** ）。
...
- ``params`` : 体素级提取器参数字典
  ...
  - **必需**: 否
  - **默认值**: ``{}``
  - **说明**: 传给 ``method`` 表达式中各提取器的键值（如 ``voxel_radiomics`` 的 ``params_file`` 也可写在 ``params`` 内，依实现解析）。无额外参数时可 **省略** 整块 ``params`` （默认为空字典）。
  - **可用方法及参数**:
...
**voxel_radiomics(...)**:
      ...
      - **参数**:
        ...
        - ``kernelRadius`` (int, CT 生境推荐: ``3``; 代码未配置时默认 ``1``): 局部邻域半径（1=3×3×3, 3=7×7×7）
      ...
      - **体素 GLCM 注意**: 请使用 ``config/radiomics/params_voxel_radiomics.yaml`` （显式列出 21 个稳定 GLCM 特征）。
        ...
        较小邻域内大量体素灰度均匀时，GLCM 退化为 1×1 矩阵，**MCC / Imc1 / Imc2**
        的特征值或互信息计算会在 CUDA/MKL 上崩溃或产生 NaN。HABIT 在检测到未限制 GLCM 时会自动替换为
        上述 21 个稳定特征并记录 warning；若在 ``params_file`` 中已显式列出特征则尊重用户配置。

      - **CT 体素纹理（R3B12）**: 对 CT 生境体素组学，文献推荐 ``kernelRadius: 3`` 与 ``binWidth: 12`` HU
        （R3B12 配置，可重复性与对核半径/分箱的稳健性优于 R1B25）。参数文件见
        ``params_voxel_radiomics.yaml``；邻域半径在 habitat 配置的 ``voxel_level.params`` 中设置。
        参考文献：Petersen A, et al. Identification of Precise 3D CT Radiomics for Habitat Computation
...
- ``params`` : 参数
  ...
  - **必需**: 否
  - **默认值**: ``{}``
  - **说明**: 传递给特征提取器的参数。``mean_voxel_features()`` 等无参方法可省略 ``params`` 。``supervoxel_radiomics`` 常用键：
...
- **适用模式**:
  ...
  - ``two_step`` ：本块配置 **个体级** 体素 → 超体素聚类（``algorithm`` 、``n_clusters`` 等在此生效）。
  - ``one_step`` ：本块 ``algorithm`` 与 ``one_step_settings`` 用于 **个体级** 体素 → 生境；``n_clusters`` 在自动选 k 时被覆盖。
  - ``direct_pooling`` ：不使用本块做超体素聚类（可保留 schema 默认值）。
...
- ``algorithm`` : 聚类或分割算法
  ...
  - **可选值** （与 ``SupervoxelClusteringConfig`` 一致）:
    ...
    - ``kmeans`` : K-means；仅按特征向量聚类，超体素可能在空间上较分散。
    - ``gmm`` : 高斯混合；底层实现默认 ``covariance_type='full'`` ，当前 ``SupervoxelClusteringConfig`` **未** 声明该字段，YAML 中填写可能被忽略。
    - ``slic`` : SLIC 超像素；联合多通道特征与 3D 空间坐标生成空间紧凑的超体素。**专用参数** （``compactness`` / ``sigma`` / ``enforce_connectivity`` 及调参说明）见下文 :ref:`habitat_slic_config` 。

- ``n_clusters`` : 超像素（或 SLIC ``n_segments``）数量

  - **类型**: 整数
  - **默认值**: ``50``
  - **说明**:

    - ``two_step`` ：每个被试固定的超体素个数；须小于 ROI 体素数。常用 **30–100**；ROI 较小时可降至 **20–30**。
    - ``one_step`` 且 ``fixed_n_clusters`` 为空时：作为自动选 k 搜索的上界参考（见 ``one_step_settings.max_clusters``），非最终固定值。
...
- ``max_iter`` : 最大迭代次数
  ...
  - **类型**: 整数
  - **默认值**: ``300``
  - **说明**: ``kmeans`` / ``gmm`` 为 sklearn 最大迭代次数；``slic`` 含义不同，见 :ref:`habitat_slic_config` 。

- ``n_init`` : 初始化次数

  - **类型**: 整数
  - **默认值**: ``10``
  - **说明**: ``kmeans`` / ``gmm`` 为 sklearn ``n_init`` ；``slic`` 仅在 ``one_step`` 自动选 k 时使用，见 :ref:`habitat_slic_config` 。

- ``compactness`` / ``sigma`` / ``enforce_connectivity`` : 仅 ``algorithm: slic`` 时生效，见 :ref:`habitat_slic_config` 。

- ``one_step_settings`` : 嵌套的 one-step 自动簇数选择（见下文 **one_step_settings** ）。**``two_step`` 下个体超体素聚类不使用此块** （two-step 始终用 ``n_clusters`` 固定个数）。
...
当 ``HabitatSegmentation.supervoxel.algorithm: slic`` 时，habit 在 ROI 内调用
``skimage.segmentation.slic`` ，将 **多通道体素特征** （最后一维为 feature channel）
与 **3D 空间坐标** 联合分割为 ``n_clusters`` 个超体素。体素各向异性间距 ``spacing``
由 mask 对应 NIfTI 的头信息 **自动读取**，**无需** 写入 YAML。

**适用场景**

- 推荐用于 ``two_step`` 第一阶段：超体素在空间上更紧凑，优于纯 K-Means 超体素。
- ``one_step`` 也可选 ``slic`` ，此时配合 ``one_step_settings`` 做逐被试自动选 k。
- 高维 ``voxel_radiomics`` 特征时计算与内存开销显著高于 ``raw`` / ``mean_voxel_features`` （需在 ROI bbox 内构建 ``[Z, Y, X, n_features]`` 特征体）。

**YAML 路径**: ``HabitatSegmentation.supervoxel`` 下下列键；仓库模板
``config/habitat/config_habitat_two_step_supervoxel_slic.yaml`` 。

**参数说明**

- ``n_clusters`` : 目标超体素个数（``n_segments``）

  - **默认值**: ``50`` （与 ``supervoxel`` 块共用）
  - **调参**: ROI 小用 **20–30** ；需更细纹理用 **60–100** ；须 **小于** ROI 体素总数。

- ``compactness`` : 特征相似度与空间紧凑度的权衡

  - **类型**: 浮点数
  - **默认值**: ``0.1``
  - **说明**: 传给 ``skimage.segmentation.slic`` 。habit 中特征通常经 winsorize / minmax 等归一化后再送入 SLIC，故 **0.1** 为合理起点（与 RGB 图像常用较大 compactness 的数值尺度不同）。
  - **调参**:

    - **增大** （``0.15``–``0.3``）：超体素更偏空间成团、边界更规整，有利于减少碎块。
    - **减小** （``0.05``）：更跟随特征差异，边界更贴强度 / 纹理变化，但可能更碎。

- ``sigma`` : SLIC 前高斯平滑宽度

  - **类型**: 浮点数
  - **默认值**: ``0.0`` （不平滑）
  - **说明**: 分割前对 ROI 内 **多通道特征体** 做高斯平滑，单位为 **体素** （结合自动 ``spacing`` 处理各向异性）。用于抑制体素级 / 纹理特征的高频抖动；**不能** 替代 ``preprocessing_for_subject_level`` 中的 winsorize 等表格级预处理。
  - **调参**:

    - ``0.0`` ：保留最锐利边界；预处理已较强时可用。
    - ``0.3``–``0.5`` ：轻度压噪声；**voxel radiomics** 等高频特征时可优先尝试（如 ``0.5``）。
    - ``> 1.0`` ：易抹平真实小结构，一般不建议。

- ``enforce_connectivity`` : 是否强制空间连通

  - **类型**: 布尔
  - **默认值**: ``true``
  - **说明**: 为 ``true`` 时合并孤立小区域，避免「飞点」超体素；医学 ROI **建议保持 true** 。

- ``max_iter`` : SLIC 迭代次数

  - **类型**: 整数
  - **schema 默认**: ``300`` （与 kmeans 共用字段名；**SLIC 请勿沿用 300**）
  - **推荐**: ``10`` （对应 ``max_num_iter``）；可试 ``15``–``20`` ，收益递减。

- ``n_init`` : 初始化次数

  - **类型**: 整数
  - **默认值**: ``10``
  - **说明**: **仅** ``one_step`` 模式下 ``find_optimal_clusters`` 内嵌 KMeans 评估时使用；``two_step`` 下对 SLIC ``fit`` **无影响** 。

- ``random_state`` : 随机种子

  - **默认值**: ``null`` （继承顶层 ``random_state``）；建议显式写 ``42`` 以保证可重复。
...
.. list-table::
   ...
   * - ``kmeans``
     ...
     - 不约束空间邻接，超体素可能空间分散
   * - ``slic``
     - 超体素空间紧凑；适合 two-step 局部单元
     - 高维 radiomics 更慢、更占内存；需 mask / 坐标

**推荐配置示例** （``two_step`` 超体素层）:
...
**one_step_settings 说明**: 嵌套块仅 ``one_step`` 使用；``two_step`` 下 SLIC 超体素个数由 ``n_clusters`` 固定，**不要** 依赖 ``one_step_settings`` 选 k。
...
- **类型**: 布尔值
- **默认值**: ``true`` （内置默认）；仓库 ``config/habitat/*.yaml`` 示例多为 ``false`` ，可按机器内存自行选择
...
- **类型**: 布尔值
- **默认值**: ``false``
- **说明**: 与 ``resume: true`` 联用。为 ``true`` 时，若 ``manifest.json`` 的 ``config_hash`` 或 ``run_mode`` 与当前 YAML 不兼容，抛出 ``CheckpointConfigHashError`` 并保留 checkpoint 目录；为 ``false`` （默认）时记录警告并删除 checkpoint 后 fresh 重跑。仅 Stage-2 配置变更导致的 legacy hash 迁移仍允许续训。
- **不纳入 config_hash** ；续训时可改。
...
本节为 **特征提取** 配置。CLI： ``habit extract -c <yaml>``。Demo 示例： ``config/feature_extraction/config_extract_features_demo.yaml``。
...
本节为 **机器学习** 配置。CLI： ``habit model -c <yaml>`` （K 折： ``habit cv``）。Demo 示例 ``config/machine_learning/config_machine_learning_radiomics.yaml``；预测见 ``config/machine_learning/config_machine_learning_predict.yaml``。
...
- **默认值**: ``train``
- **可选值**: ``train`` 、``predict``
- **说明**: 训练与预测共用同一套机器学习配置结构。``predict`` 时必须提供 ``pipeline_path``；``models`` 在预测模式下被忽略。
...
- ``params`` : 方法特定参数
  ...
  - **类型**: 字典
  - **默认值**: ``{}``
  - **说明**: 根据选择的归一化方法传递不同的参数。使用默认行为时（如 ``z_score`` 、``max_abs``）可省略 ``params`` 。
  - **各方法支持的参数**:
...
本节为 **Test-Retest 重复性** 配置。示例： ``config/auxiliary/config_test_retest.yaml``。用户指南：:doc:`app_habitat_test_retest_zh`。
...
本节为 **传统组学** 配置。示例： ``config/radiomics/config_traditional_radiomics.yaml``。
...
完整场景说明与测试脚本路径见 **`config/README_CONFIG.md`** （仓库根目录，与 ``habit/`` 包同级）。
...
1. 从仓库根目录 ``config/`` 复制对应场景的示例 YAML 并修改（场景列表见 ``config/README_CONFIG.md`` ）
2. 参考本文档各小节字段说明，在副本上增删参数
3. 从零新建 YAML（不推荐，易漏必填项）
