...
数据位置由各流程 YAML 的 ``data_dir``（或特征提取的 ``raw_img_folder`` 等）指定；``data_dir`` 可为目录或路径清单 YAML：

- **文件夹方式** ：``data_dir`` 指向固定 ``images/``、``masks/`` 结构根目录
- **YAML 清单方式** ：``data_dir`` 指向路径清单文件（推荐，详见 :doc:`data_structure_zh`）
