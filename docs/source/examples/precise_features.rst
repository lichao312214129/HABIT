...
``DATA`` / ``MODALITIES`` / ``ROI``) and uses raw intensities so it runs
without PyRadiomics; with the default voxel-radiomics factory the same
call is the full paper design.
...
``demo_data`` subject with raw LAP intensity, not a clinical claim)::

   === precision screen (repeatability experiment) ===
     precise (stable) features: ['LAP']
     unstable features: []
      experiment feature  value   lcl   ucl  n_voxels  precise
   repeatability     LAP   0.88 0.877 0.882   31814.0     True

   === whitelist bridge into a habitat spec ===
     whitelist spec: {'name': 'feature_whitelist', 'params': {'features': ['LAP']}, 'version': '1.0'}
     habitat maps: 1

   === artefact round trip ===
     saved precise_features.json; reloaded features: ['LAP']

Figures
-------

The screen's scientific figure is the LCL panel (blue = precise / stable,
gray = unstable); the overlay is the habitat map after clustering
**only** those precise features. The perturbation panels below are the
**same** Prior 2024 / MIRP 1.2.0 components applied to one
``demo_data/preprocessed`` subject (not a clinical claim). Regenerated by
``python docs/source/examples/scripts/precise_screening_tutorial_demo.py``.
...
.. figure:: ../_static/images/examples/precise_features_icc_lcl.png
   ...
   0.5 LCL threshold (matplotlib; the screen's tabular product). Demo
   subject; values depend on the image and extractor.
...
Voxel and habitat stability (print + figures)
---------------------------------------------

The screen above **prints** precise vs unstable feature names and colours
the ICC bars the same way. Two spatial follow-ups live in the perturbation
script (same ``DATA`` / ``MODALITIES`` / ``ROI``):

* **ROI unchanged** (Prior 2024 retest: intensity + small geometry, mask
  the same) — voxel entropy on the original vs the retest, then a
  stable vs unstable voxel map (75th-percentile absolute difference
  inside the ROI; a demo threshold, not the paper's ICC rule).
* **ROI shape changes** (:class:`~habit.domain.precision.BSplineDeformPerturbation`
  with ``target_dice``) — one-step habitats before vs after, Hungarian
  per-habitat Dice from :func:`~habit.domain.precision.habitat_stability`,
  and ITH / MSI / graph / volume features marked stable vs unstable by
  relative change.

Paste after the Script block (it uses ``subject``, ``retest``,
``ffd_subject``, ``deformed``, and the entropy maps). Writes
``out/precise_voxel_stable_vs_unstable.png``,
``out/precise_habitat_stability_compare.png``,
``out/precise_habitat_dice.png``, and
...
.. figure:: ../_static/images/examples/precise_voxel_stable_vs_unstable.png
   :alt: Original vs retest entropy and stable versus unstable voxels
   :width: 720

   Prior-style voxel display while the ROI is unchanged: original
   entropy, retest entropy, and voxels labelled stable (green) vs
   unstable (vermillion).

.. figure:: ../_static/images/examples/precise_habitat_stability_compare.png
   :alt: Habitat maps before and after B-spline ROI warp
   :width: 720
...
.. figure:: ../_static/images/examples/precise_habitat_feature_stability.png
   :alt: Habitat-level ITH MSI graph volume stability bars
   :width: 520

   ITH / MSI / graph / volume: green = stable, vermillion = unstable
   (relative change 0.15 cutoff; demo heuristic).
