...
same 8-voxel lattice as dashed lines. Graph topology is a
...
off). One ``fit_predict`` on three subjects feeds the overlay, the 2D
network, and the heatmap table. Figures land under ``out/`` (swap that
...
``out/graph_feature_heatmap_pair.png``. Heatmap knobs
