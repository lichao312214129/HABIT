...
``y_pred``, ``FEATURES``, ``imp_mean``, ``imp_std``, ``cv``,
``holdout_table``, and ``reloaded``). Writes ``out/tabular_ml_*.png``.
SHAP panels need ``habitat-analysis[explain]``.
...
.. figure:: ../_static/images/examples/tabular_ml_shap_summary.png
   ...
   Hold-out SHAP beeswarm (:func:`~habit.viz.plot_shap_summary`).
