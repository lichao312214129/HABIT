...
A leaf is ``Spec("raw", ...)``; mixed families are a tree from
:func:`~habit.spec.parse_feature_expression` (section 1)::
...
field. There are **three** ways to fill this slot — pick the simplest that
matches the science. Mixed families (raw on one series, texture on
another) need a **tree**, not a longer ``modalities`` list.

A. One family, one or more modalities
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

A single leaf already accepts a modality list. Same extractor, one column
(or one block of columns) per series. No ``concat`` needed::

   # One series
   Stage("extract_voxel_features", Spec("raw", {"modalities": ["T1"]}))
   Stage(
       "extract_voxel_features",
       Spec("local_entropy", {"modalities": ["T1"], "kernel_size": 3, "bins": 32}),
   )

   # Same family on several series (columns T1, T2, …)
...
B. Different families, then concat
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

``concat`` joins **different extractors** on the same voxels (same ROI).
Two equivalent spellings — expression string or nested ``children`` —
share one fingerprint. Modalities in the expression form are **quoted**.

**raw T1 + voxel radiomics T2** (texture habitats on T2, intensity on T1)::
...
C. Nested combiners (weights, ratios, aliases)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Combiners nest. Built-ins: ``concat``, ``weighted_concat``
(``weights=[...]``), ``average``, ``ratio``, ``difference``, ``kinetic``,
``expression``. ``as_="label"`` renames a **single-column** leaf so two
branches do not collide::
   ...
   Stage(
       ...
       parse_feature_expression(
           ...
           'local_entropy("T1", kernel_size=3))'
       ),
   )
...
Worked trees and column names: :doc:`../examples/feature_composition`.
...
A. Default — mean of the voxel field
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

``mean_voxel_features`` averages whatever columns the voxel stage already
produced (raw T1+T2, or a concat tree). Omit this stage unless you need
something else; many two-step recipes rely on the partition's attached
means::
...
B. Per-modality statistics, then concat
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

``mean`` / ``std`` / ``percentile`` are **one modality each**. Compose
them the same way as voxel leaves. ``source="working"`` (default) uses
the preprocessed voxel field; ``source="original"`` uses the raw series
the partition saw::
...
Or structured ``children`` (same fingerprint) as in
:doc:`../examples/feature_composition`.

C. Supervoxel radiomics (not a voxel kernel)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

``supervoxel_radiomics`` extracts **whole-region** texture per supervoxel
label (not a sliding voxel kernel). Single series still sits in
``concat`` when you mix families::
...
Needs ``pyradiomics``. Values differ from ``voxel_radiomics`` (different
spatial support).
