...
1. **肿瘤异质性**: 肿瘤不是均匀的组织，而是由多个具有不同特征的亚区域组或
...
*   **五颜六色的区域是什么？**
    每一个颜色代表一个“生境”（Habitat）。例如，红色可能代表 Cluster 1，蓝色代行Cluster 2。
*   **如何赋予临床意义：**
    您需要结合原始影像的信号特征来解读。
    *   如果 Cluster 1 在T1 增强序列上信号很高，在ADC 图上信号很低，它可能代表 **活性肿瘤区**。
    *   如果 Cluster 2 在T2 序列上信号高，无强化，可能代行**坏死化* 或**水肿化*。

2. 聚类验证图(visualizations/optimal_clusters/)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

该图用于所有需要自动选择聚类数的场景（包择One-Step 和Two-Step 的相应聚类阶段）。

*   **横轴**：尝试的聚类数量（k=2, 3, 4...）。
*   **纵轴**：聚类评价指标（如Inertia 或Silhouette Score）。
*   **如何看？**
*   **Inertia/Kneedle**：``inertia`` 是不合k 下的簇内平方误差曲线；``kneedle`` 是在该曲线上自动寻找拐点的策略。当前实现中，选择 ``inertia`` 或``kneedle`` 都会基于 Inertia 曲线使用 Kneedle 选拐点。
...
.. code-block:: yaml
   ...
   HabitatSegmentation:
     ...
     habitat:
       ...
       # - 'inertia'。kneedle': 都是在Inertia 曲线上使用Kneedle 选拐点
   ...
   # 通用设置
   processes: 2  # 并行进程数
   individual_subject_timeout_sec: 1800  # 个体级并行单被试时长上限（秒），默认 30 分钟；null 表示不限制
...
- 可以是文件夹或YAML 配置文件
- 参者:doc:`../data_structure_zh` 了解数据结构
...
- **类型**: 数值（秒）或 ``null``
- **默认**: ``1800``（30 分钟）；在 YAML 中省略该键时与代码默认值一致。
...
配置时把对应的key 填到 ``HabitatSegmentation.supervoxel.algorithm`` 或
...
录``fixed_n_clusters`` 未设置时，HABIT 在``min_clusters`` 个
``max_clusters`` 区间内枚个``k``，每个``k`` 计算评分并按下表逻辑选最会k。
...
.. list-table::
   ...
   * - ``kneedle``
     - Kneedle 拐点
     - 显式使用 Kneedle；当前实现与 ``inertia`` 共用同一来inertia 曲线。
   * - ``bic`` / ``aic``
     - 取最将
     - 信息准则，主要用于GMM。
   * - ``gap``
...
每个方法独立选出最会k，按得票投票，最终选得票最多的 k；票数相同时取较小的
k（更保守）。例如``[silhouette, calinski_harabasz, davies_bouldin]`` 个
两种选5、一种选4，则结果是5。
...
为了减少生境图中“破碎小块”，HABIT 支持在ROI 内执行连通域后处理。当前实现采用**SimpleITK 快路征*，

1. 按标签识别并临时移除小连通域（体素数小于 ``min_component_size``）。
2. 使用最近的大连通域种子标签对被移除体素进行回填。

该流程的关键目标是：**减少碎块，同时保指ROI 内体素不丢失**。
...
- ``debug_postprocess``: 是否输出后处理详细日志（按标等阶段，
...
1. **训练阶段**: 在训练集上训组Pipeline，包括特征提取、聚类等步骤
2. **预测阶段**: 加载训练好的 Pipeline，应用于测试集，确保使用相同的处理流程
3. **严格分离**: 训练集和测试集完全分离，避免测试集信息泄需
...
``predict()``（``run()`` 仍可按配置分发，但已标注为遗留入口）。若要直接操级
序列化后的``HabitatPipeline``，可使用个sklearn 一致的接口，
...
.. code-block:: python
   ...
   #       并把工fit 的pipeline 保存分habitat_pipeline.pkl
...
- ``HabitatPipeline.fit(X)``：在训练数据上学习参数，包括 group 级预处理统计重
  个habitat 聚类模型；调用后 ``fitted_=True``。
- ``HabitatPipeline.transform(X)``：仅在``fitted_=True`` 时可用；用已学到的
  参数处理新数据。
- ``HabitatPipeline.save(path) / .load(path)``：joblib 序列化整来pipeline，
  反序列化时所有step 的状态（``PreprocessingState``、聚类模型、最会k，
  都会恢复。
...
- **小样本研空**: 可以考虑 One-Step 策略，关注每个病例的独特性。
...
- **kneedle**: 显式使用 Kneedle 拐点检测；当前实现个``inertia`` 使用同一来Inertia 曲线。
- **silhouette**: 使用轮廓系数。
