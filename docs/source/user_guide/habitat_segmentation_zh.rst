...
- **说明**: ``persistent`` （默认）：每个 worker 槽位一个长生命周期子进程，在同一 ``train`` 运行内（含 auto-retry）复用，减少重复 import/spawn；单 GPU 时仍为串行，主要摊销启动成本。``isolated`` ：每被试单独 spawn 子进程（pipeline 无法 pickle 或需更强隔离时使用）。``processes=1`` 且 ``individual_subject_timeout_sec: null`` 时不 spawn。``predict`` 模式忽略。
...
- **不纳入 hash**（改后可 ``resume: true`` 复用 pkl）：``preprocessing_for_group_level`` 、``two_step``/``direct_pooling`` 的 ``habitat.*`` （群体聚类）、``processes`` 、``individual_subject_timeout_sec`` 、``individual_subject_auto_retry_rounds`` 、``individual_subject_parallel_mode`` 、``persistent_worker_max_consecutive_failures`` 、``persistent_worker_recycle_after_tasks`` 、``retry_failed_subjects`` 、``plot_curves`` 、``out_dir`` 等。
...
- ``failed_subjects`` ：超时、崩溃或 Python 异常的被试；**下次**续训启动时默认跳过（除非 ``retry_failed_subjects`` / ``force_rerun_subjects``）；**同一次** train 内默认由 ``individual_subject_auto_retry_rounds`` 自动重试
...
同一次 train 运行内（默认，无需改 YAML）：

.. code-block:: yaml

   individual_subject_auto_retry_rounds: 2  # 首轮后再最多重试 2 轮；0=关闭

若 pipeline 无法 pickle 或需排查 spawn 问题，可改回 ``isolated`` ：
...
.. list-table::
   ...
   * - ``Resume: skipping M previously failed subject(s)``
     - M 个失败被试被跳过（需 ``retry_failed_subjects`` 或 ``force_rerun_subjects`` 才能在下一次启动时重跑）
   * - ``Auto-retry round R/N for M failed subject(s): ...``
     - 同一次 train 内 Stage 1 自动重试（``individual_subject_auto_retry_rounds``）
     - persistent worker 池（``individual_subject_parallel_mode`` 默认 ``persistent``）
   * - ``Auto-retry exhausted (N round(s)); M subject(s) still failed: ...``
     - 自动重试轮次用尽，仍有 M 个被试失败
...
- 保存 pkl 时会**自动瘦身**：去掉训练集 ``labels_``、清空 ``mask_info_cache``、不序列化 checkpoint 管理器。预测所需的是聚类模型与 ``PreprocessingState``。
...
- `kinetic`: 动力学特征
- `local_entropy`: 局部熵
- `voxel_radiomics`: 体素级影像组学特征（需 ``params_file``；可选 ``kernelRadius``、``voxelBatch``）
- `mean_voxel_features`: 平均体素特征
- `supervoxel_radiomics`: 超体素 ROI 影像组学（union-mask 一次 bin；见下文）

**体素级组学参数（``voxel_radiomics``）：**

写在 ``FeatureConstruction.voxel_level.params`` 中；**不必**全部出现在 ``method`` 表达式里（如 ``voxelBatch``、``useTorchRadiomics`` 会自动合并转发）。

- ``params_file``: PyRadiomics 参数 YAML（必需；体素级推荐 ``config/radiomics/params_voxel_radiomics.yaml``）
- ``kernelRadius``: 邻域半径，默认 ``1``（1=3×3×3）
- ``voxelBatch``: 批大小，默认 ``1000``；``-1`` 表示不分批（PyRadiomics 原生）；大 ROI 或 GPU 时可设为 ``512`` 等以控制内存
- ``useTorchRadiomics``: ``auto`` / ``true`` / ``false``，默认 ``auto``
- ``torchGpus``: 允许使用的 GPU 列表，如 ``[0, 1, 2]``
- ``torchGpuCount``: 实际使用前 N 张 GPU
- ``torchDevice``: 单卡模式（未设置 ``torchGpus`` 时）
- ``torchDtype``: Torch dtype，默认 ``float32``（需严格对标 CPU PyRadiomics 时可设 ``float64``）

**体素 GLCM 配置（重要）**

体素级提取在每个体素周围仅取 ``kernelRadius`` 定义的小邻域（如 ``1`` → 3×3×3）。
大量邻域灰度几乎相同，GLCM 矩阵退化为 1×1；若 ``params_file`` 中 ``glcm:`` 未显式列出特征名，
PyRadiomics 会计算全部 24 个 GLCM 特征，其中 **MCC、Imc1、Imc2** 易在 CUDA/MKL 特征值分解时崩溃。
请使用 ``config/radiomics/params_voxel_radiomics.yaml``（21 个稳定 GLCM 特征，排除上述三项）。
HABIT 在检测到未限制的 ``glcm`` 时会自动替换为相同列表并输出 warning。
...
对超像素区域提取特征。``mean_voxel_features()`` 聚合体素级特征；
``supervoxel_radiomics`` 对每个 label 做 **整 ROI** PyRadiomics 纹理提取。

**适用场景：**

- 需要区域级别的分析
- 减少计算复杂度（相对体素级组学）
- 需要独立于 ``voxel_level`` 的超体素纹理特征

**supervoxel_radiomics 实现要点（habit）：**

- **离散化**: 全部超体素并集（``sv_map > 0``）上 **一次** PyRadiomics bin（与体素级整 mask bin 同类思路）
- **矩阵**: 每个 label 调用 PyRadiomics ``cMatrices`` ROI 路径（``_initCalculation``）
- **加速**: ``useTorchRadiomics: auto`` 且有 CUDA 时用 TorchRadiomics GPU；否则 CPU PyRadiomics
- **批大小**: ``supervoxelBatch`` 默认 ``64``（分组处理 label，非进度条、非 kernel 半径）
- **继承**: ``supervoxel_level`` 未写的 ``useTorchRadiomics`` / ``torchGpus`` 等可从 ``voxel_level.params`` 继承
- **注意**: ``kernelRadius`` 仅用于 ``voxel_radiomics``；数值与旧版逐 label ``execute``（每 label 各 bin）不同
...
.. code-block:: text
   ...
   results/habitat/
   ├── train/                      # 训练模式输出
   ─  ├── habitat_pipeline.pkl    # 已拟合 Pipeline（save 时自动瘦身，不含 mask 缓存）
   ─  ├── habitats.csv            # 生境结果表（如启用 save_results_csv）
   ─  ├── subj001_habitats.nrrd   # 生境图（save_images: true；One-Step/Direct Pooling）
...
**Q4: ``habitat_pipeline.pkl`` 为什么很大（例如数 GB）？**

A: 常见原因与处理方式：

1. **历史版本**：旧 pkl 可能在 ``save_images: true`` 时内嵌各被试 ``mask_array`` ，222 例时可达数 GB。
2. **当前版本**：``save()`` 会去掉训练 ``labels_`` 且 **不写入 mask**；写 NRRD 时从 ``data_dir`` 加载。重新 train 后 pkl 通常为几十～几百 MB。
3. **若不需要 NRRD**：设 ``save_images: false`` ，仅输出 ``habitats.csv`` 。
4. **群体寻优 k 的进度**：日志中会输出 ``Trying N cluster(s) [i/total]`` 与 ``Cluster search finished`` ，便于判断 Stage 2 是否仍在运行。
