...
该图用于所有需要自动选择聚类数的场景（包括 One-Step 和 Two-Step 的相应聚类阶段）。
...
配置时把对应的 key 填到 ``HabitatsSegmention.supervoxel.algorithm`` 或
``HabitatsSegmention.habitat.algorithm`` 即可。
...
.. list-table::
   ...
   * - Gaussian Mixture
     - ``gmm``
     - 概率模型；可输出后验概率，支持椭球簇与 AIC/BIC 选数。
   * - SLIC
     - ``slic``
...
当 ``fixed_n_clusters`` 未设置时，HABIT 在 ``min_clusters`` 与
...
.. list-table::
   ...
   * - ``kneedle``
     - Kneedle 拐点
     - 显式使用 Kneedle；当前实现与 ``inertia`` 共用同一条 inertia 曲线。
   * - ``bic`` / ``aic``
     - 取最小
     - 信息准则，主要用于 GMM。
   * - ``gap``
...
.. code-block:: python
   ...
   #       并把已 fit 的 pipeline 保存到 habitat_pipeline.pkl
...
- ``HabitatPipeline.fit(X)``：在训练数据上学习参数，包括 group 级预处理统计量
  与 habitat 聚类模型；调用后 ``fitted_=True``。
- ``HabitatPipeline.transform(X)``：仅在 ``fitted_=True`` 时可用；用已学到的
  参数处理新数据。
- ``HabitatPipeline.save(path) / .load(path)``：joblib 序列化整条 pipeline，
  反序列化时所有 step 的状态（``PreprocessingState``、聚类模型、最优 k）
  都会恢复。
- 在 ``HabitatAnalysis.predict()`` 加载的 pipeline 上，运行时的 service
...
- **kneedle**: 显式使用 Kneedle 拐点检测；当前实现与 ``inertia`` 使用同一条 Inertia 曲线。
- **silhouette**: 使用轮廓系数。
- **calinski_harabasz**: 使用 Calinski-Harabasz 指数。
- **davies_bouldin**: 使用 Davies-Bouldin 指数。

多方法自动选择时，请使用 YAML 列表，例如 ``[inertia, silhouette, calinski_harabasz]``。系统会让每个方法各自投票，得票最多的 k 被选中；如果平票，选择较小的 k。
