...
.. code-block:: yaml
   ...
   save_images: true  # 是否保存 *_habitats.nrrd；false 可显著缩小 habitat_pipeline.pkl
...
- **类型**: 布尔值，默认 ``true``
- **说明**: 为 ``false`` 时仍可输出 ``habitats.csv`` ，但 predict 默认不写 NRRD；且 ``habitat_pipeline.pkl`` 不含 mask 缓存，体积更小（见下文「与最终产物的关系」）。
...
- 保存 pkl 时会**自动瘦身**：去掉训练集 ``labels_``、去掉 mask 的 SimpleITK 重复对象、不序列化 checkpoint 管理器。预测所需的是聚类模型与 ``PreprocessingState``。
- **pkl 体积**：``direct_pooling`` 且被试多时，若 ``save_images: true`` ，pkl 仍可能为 **数 GB**（含各被试 ``mask_array`` 以供 predict 写 NRRD）。若只需 CSV、不写 NRRD，设 ``save_images: false`` 可显著缩小。旧 pkl 需重新 train 后才会应用瘦身。
...
.. code-block:: text
   ...
   results/habitat/
   ├── train/                      # 训练模式输出
   ─  ├── habitat_pipeline.pkl    # 已拟合 Pipeline（save 时自动瘦身；direct_pooling+save_images 仍可能数 GB）
...
1. **``direct_pooling`` + 被试多**：pkl 在 ``save_images: true`` 时会保留各被试 ``mask_array`` ，供 predict 写 NRRD；222 例时可达数 GB。
2. **已自动瘦身**：当前版本 ``save()`` 会去掉训练 ``labels_`` 与 SimpleITK ``mask`` 重复数据；旧 pkl 需重新 train。
3. **若只需 CSV、不写 NRRD**：在 YAML 中设 ``save_images: false`` ，pkl 通常可降至几十～几百 MB。
