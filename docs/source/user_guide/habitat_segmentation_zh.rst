...
- **不纳入 hash**（改后可 ``resume: true`` 复用 pkl）：``preprocessing_for_group_level`` 、``two_step``/``direct_pooling`` 的 ``habitat.*`` （群体聚类）、``processes`` 、``individual_subject_timeout_sec`` 、``individual_subject_auto_retry_rounds`` 、``retry_failed_subjects`` 、``plot_curves`` 、``out_dir`` 等。
