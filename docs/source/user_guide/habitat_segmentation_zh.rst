...
- ``torchGpus``: 允许使用的 GPU 列表，如 ``[0, 1, 2]`` ；当 ``cap_processes_to_gpu_pool: true`` （默认）时，Stage 1 worker 数不超过该列表长度
...
- **矩阵**: 每个 label 调用 PyRadiomics ``cMatrices`` ROI 路径（``_initCalculation``）；``useSupervoxelCext: auto`` 时优先用 habit C 扩展批量建矩阵
...
- **C 扩展**: ``useSupervoxelCext`` 默认 ``auto``（``auto`` / ``true`` / ``false``）；``auto`` 在已编译扩展时启用 C 批量矩阵，否则回退原有 Torch/PyRadiomics 路径；``false`` 强制禁用 C 扩展（``matrix_backend=torch_cmatrices``）
