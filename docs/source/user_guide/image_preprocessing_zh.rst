...
- **若需 Elastix 可变形配准**：设置 ``backend: elastix``，安装官方 **elastix** / **transformix** 并在 YAML 中提供 ``elastix_parameter_files``（及可选 ``elastix_path`` 等）。无需 ANTsPy。通过 ``elastix_parameter_overrides`` 可覆盖参数。并行被试由顶层 ``processes`` 控制，与其它后端相同。
