...
- **配准（``registration``）**：默认 ``backend: ants`` 时需 **ANTsPy**（及底层 ANTs）；若设置 ``backend: simpleitk`` 或 ``backend: elastic`` 则仅用 **SimpleITK**，可不装 ANTsPy。仅使用重采样 / 标准化等步骤、不配准时也可不装 ANTsPy。
...
- **resample**：重采样至目标体素间距
- **registration**：多时相配准到参考图像（``backend: ants`` 用 ANTsPy，``backend: simpleitk`` / ``backend: elastic`` 用 SimpleITK）
...
``RegistrationPreprocessor`` 支持 ``backend: ants``（默认）、``backend: simpleitk`` 与 ``backend: elastic``。前两者在 YAML 中共享 ``images``、``fixed_image``、``type_of_transform``、``metric``、``optimizer``、``use_mask``、``replace_by_fixed_image_mask`` 等入口键；``elastic`` **忽略** ``type_of_transform``（固定为刚性 + B 样条两阶段），其余调键与 SimpleITK 系相同；**高级调参** 因后端而异（见下文）。

**公共约定**

- **浮动序列**：``images`` 中除 ``fixed_image`` 以外的每个模态都会依次配准。请勿使用 ``moving_images``，实现不会读取该键。
- **掩膜**：``use_mask`` 为真且存在 ``mask_<modality>`` 时参与度量；``replace_by_fixed_image_mask`` 为真时，配准后用固定侧掩膜替换浮动侧掩膜键。否则掩膜经变换对齐到固定网格：ANTS 路径用 ``ants.apply_transforms``；SimpleITK 路径用 ``ResampleImageFilter``；``elastic`` 路径按 **刚性变换 → B 样条变换** 顺序链式重采样（与强度图两阶段一致）。
...
**仅 simpleitk / elastic 后端读取的 YAML 键** （与同名的 ANTs 迭代键 **无关**；这些键在 ``backend: ants`` 时会被 **剥离**，不传入 ``ants.registration``）：
...
实现中键名常量见源码 ``habit/core/preprocessing/registration.py`` 中的 ``_SITK_OPTION_KEYS``；**``backend: elastic`` 时这些键同样生效**（与 simpleitk 一致）。

**backend: elastic**

- **依赖**：仅需 **SimpleITK**，**不**\ 导入 ANTsPy。
- **行为**：先做 **刚性**（``VersorRigid3DTransform`` / ``Euler2DTransform``）将浮动图像对齐到固定图像网格并写出刚性 ``.tfm``，再在刚性重采样后的浮动图像上做 **B 样条** 可变形配准并写出第二份 ``.tfm``；``data`` 中对应模态的 ``*_transform_files`` 为 **两元素列表**（先刚性、后 B 样条）。掩膜在 ``use_mask`` 且非 ``replace_by_fixed_image_mask`` 时按同一顺序链式变换。
- **``type_of_transform``**：该后端下 **不使用**（固定上述两阶段）；YAML 中可保留占位或与公共模板一致，实现中不会据此分支。
