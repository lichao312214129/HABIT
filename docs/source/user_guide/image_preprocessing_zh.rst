...
- **浮动序列**：对 ``images`` 中 **除** ``fixed_image`` 外的每个模态依次配准；**不要** 使用 ``moving_images``（实现不读取该键；若写入可能作为多余关键字传入 ANTs，造成告警或报错）。
- **掩膜**：``use_mask: true`` 且存在 ``mask_<modality>`` 时参与度量；若 ``replace_by_fixed_image_mask: true``，配准后用固定侧掩膜替换浮动侧掩膜键。否则掩膜经变换对齐到固定网格：ANTS 使用 ``ants.apply_transforms``（最近邻）；SimpleITK 使用 ``ResampleImageFilter``（最近邻）。
...
- 构造 ``reg_params = { "metric": ..., "optimizer": ..., **YAML中其余关键字}``；若 ``use_mask`` 为真且存在掩膜，则附加 ``mask``（固定图像侧）、``moving_mask``（浮动图像侧）。
- 最终等价于：\ ``ants.registration(fixed=..., moving=..., type_of_transform=..., **reg_params)``（见 ``habit/core/preprocessing/registration.py``）。
...
下列与 ``registration.py`` docstring 一致，并随 ANTsPy 版本可能增删；**完整与默认值以** ``ants.registration`` **文档为准**。
...
- ``metric``（默认 ``MI``）：写入 ``reg_params``；参见 RegistrationPreprocessor docstring（例如 ``CC``、``MI``、``MeanSquares``、``Demons``）。
- ``optimizer``：写入 ``reg_params``；例如 ``gradient_descent``、``lbfgsb``、``amoeba``（以 ANTsPy 接受为准）。

**通过 YAML 透传到 ANTs 的高级参数**

在 ``registration:`` 下除 ``images`` / ``fixed_image`` / ``backend`` 及 **仅 SimpleITK 的调优键** 外，凡 **未** 被 HABIT 单独解析的键（若与 ANTs 参数名一致）会进入 ``**kwargs`` 再合并进 ``ants.registration``。常用例子（是否可用取决于 ANTsPy 版本）：
...
- **实现概要**：``sitk.ImageRegistrationMethod`` + 多分辨率（``shrink_factors_per_level`` / ``smoothing_sigmas_per_level``）；配准得到的 ``sitk.Transform`` 写入临时目录下的 ``.tfm``，并在 ``data`` 中保持与 ANTs 路径相同的 ``<key>_transform_files`` 列表语义；掩膜对齐使用 ``sitk.ResampleImageFilter``（最近邻）。
...
**仅 ``backend: simpleitk`` 时读取的 YAML 键**（与同名的 ANTs 迭代键 **无关**；这些键在 ``backend: ants`` 时会被 **剥离**，不传入 ``ants.registration``）：
