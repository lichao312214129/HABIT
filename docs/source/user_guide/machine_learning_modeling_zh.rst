...
1. **准备特征**: 提取生境特征并保存为 CSV。
2. **准备临床数据**: 准备包含相同 PatientID 的临床特征 CSV。
3. **合并表格**: 使用工具（如 `scripts/merge_csv_files.py`）将两者按 ID 合并为一个大表。
4. **训练模型**: 在 `habit model` 的配置文件中，将合并后的文件作为 `data_dir` 输入，即可训练包含两类特征的融合模型。

**方式 2：晚期融合 (Late Fusion)**

分别训练影像模型和临床模型，然后将两者的预测概率进行加权平均或再次训练一个 Meta-Model。

1. **运行影像模型**: 得到影像模型的 `Radscore`。
2. **运行临床模型**: 得到临床模型的预测结果。
3. **加权组合**: 将两个得分按比例相加，或者作为新特征输入到一个简单的逻辑回归模型中。
