...
.. seealso::
   ...
   各命令背后的第三方库文档索引详见 :doc:`reference/upstream_libraries_zh`。

**主要命令：**

- `habit preprocess`: 图像预处理
- `habit sort-dicom`: DICOM 仅整理/重命名（dcm2niix）
...
不同命令支持的参数不同，请以 `habit <command> --help` 为准。常用参数包括：
...
- **必需**: 取决于具体命件
- **说明**: 指定 YAML 配置文件的路征
- **示例**: `habit preprocess --config config/preprocessing/config_preprocessing_demo_elastix.yaml`

**--debug**: 启用调试模式

- **类型**: 标志
- **必需**: 否
- **说明**: 启用详细的日志输出，便于调试。当前主要用于`habit get-habitat`
- **示例**: `habit get-habitat --config config.yaml --debug`
...
**功能**: 对医学图像进行预处理，包括DICOM 转换、重采样、配准、标准化等。
...
.. code-block:: yaml
   ...
   Preprocessing:
     ...
     registration:
       ...
       use_mask: false
       # backend: ants  # optional: ants | simpleitk | elastix — see user_guide/image_preprocessing_zh.rst
...
- 预处理后的图像保存在 `out_dir` 指定的目录（默认最终产物在 ``processed_images`` 下）
- 处理日志保存在 ``<out_dir>/processing.log`` （详见 :doc:`user_guide/image_preprocessing_zh`）
...
**功能**: 使用 dcm2niix 仅整理或重命名 DICOM（等价 ``-r y``），不进行 NIfTI 转换；**不属于** ``habit preprocess`` 批量预处理流水线。
...
- ``--config`` / ``-c`` : ``DicomSortConfig`` YAML（用法以 ``habit sort-dicom --help`` 为准）

**配置文件与路径规则**: 见 :doc:`user_guide/image_preprocessing_zh` 「独立命令：DICOM 整理」；``f`` / ``extra_args`` 原样传给 dcm2niix，不会被拼成「基于 YAML 目录的绝对路径」。

**输出**:

- 日志默认写在 ``output_dir`` （未设置则用 ``out_dir``）下的 ``processing.log``
...
.. code-block:: bash
   ...
   habit get-habitat --config config/habitat/config_habitat_two_step.yaml --mode train
...
.. code-block:: bash
   ...
   # 预测模式（在 YAML 中设 run_mode: predict + pipeline_path，
   habit model --config config_ml_predict.yaml --mode predict
...
少数命令支持用命令行参数覆盖配置文件中的同名设置。例如`habit get-habitat` 支持用`--mode` 和`--pipeline` 覆盖配置文件中的运行模式和pipeline 路径。
...
**habit dicom-info 主要参数** (``habit dicom-info``)（无 YAML 配置）：
...
- `habit retest`: Test-retest 重现性分析
  - 参数: `--config, -c` (可选，但未提供时会报错)
  - 功能: 评估生境映射在测试重测扫描中的稳定性
...
.. code-block:: bash
   ...
   # 查看日志文件
   cat ./preprocessed/processing.log

   # 对支持调试参数的命令使用调试模式重新运行
   habit get-habitat --config config.yaml --debug
...
- :doc:`../configuration_zh`: 了解配置文件的详细说是
- :doc:`../user_guide/index_zh`: 了解各个功能的详细使用指单
