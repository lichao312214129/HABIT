...
Demo 与多数 YAML 假定 **工作根目录** 下同时有 ``config/`` 与 ``demo_data/`` （路径因安装方式而异，**请用本机实际文件夹，勿照搬文档示例盘符**）：

- **Windows 便携包**：便携包根目录（示例 ``D:\habit-cpu\`` ）；``config.rar`` 、``demo_data.rar`` 从网盘解压到此（见 :doc:`getting_started/quickstart_zh`）
