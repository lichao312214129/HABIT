...
**Windows 用户（推荐）**：下载便携包 → 解压 → 双击 ``setup_habit.bat`` → 下载配置与演示数据，即可使用。
...
便携包 **不含** 示例配置和演示数据，需 **单独下载** 并解压到 **与 python.exe 同一文件夹**：

+------------------+---------------------+------------------------------------------+--------+
| 网盘文件         | 解压后应出现        | 链接                                     | 提取码 |
+==================+=====================+==========================================+========+
| ``config.zip``   | ``config\`` 文件夹  | |config_pack_link|                       | |config_pack_code| |
+------------------+---------------------+------------------------------------------+--------+
| ``demo_data.rar``| ``demo_data\`` 文件夹| |demo_data_link|                         | |demo_data_code| |
+------------------+---------------------+------------------------------------------+--------+
| ``tests.zip``    | ``tests\``（可选）  | |tests_pack_link|                        | |tests_pack_code| |
+------------------+---------------------+------------------------------------------+--------+

解压时选 **解压到当前文件夹**，不要多嵌套一层目录。完成后目录类似：
...
从网盘下载 ``demo_data.rar`` ，解压到项目根目录，应出现 ``demo_data\`` 文件夹（链接见方式一表格）。
