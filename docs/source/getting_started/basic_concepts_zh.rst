...
1. **训练阶段**: 在训练集上训组Pipeline，包括特征选择、聚类、模型训练等步骤
...
.. code-block:: python
   ...
   # Wired HabitatAnalysis (clustering_mode →step recipe lives inside HabitatAnalysis)
   habitat_analysis = configurator.create_habitat_analysis()

   # Prefer explicit verbs: train with fit(); load saved pipeline with predict()
...
- **参者**: Intratumoral Spatial Heterogeneity at Perfusion MR Imaging Predicts Recurrence-free Survival in Locally Advanced Breast Cancer Treated with Neoadjuvant Chemotherapy
...
参者:doc:`../data_structure_zh` 了解更多关于数据配置文件的信息。

双重接口
--------

HABIT 提供两种使用方式：CLI 和Python API。
...
- **初学者**: 件CLI 开始，快速上所
- **研究者**: 根据需求选择，CLI 用于快速实验，API 用于定制开可
