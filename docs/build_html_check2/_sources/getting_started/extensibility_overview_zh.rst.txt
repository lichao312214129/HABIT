...
- **用途**: 添加自定义的生境聚类前特征预处理（``FeatureConstruction`` 中的
  ``preprocessing_for_subject_level`` / ``preprocessing_for_group_level``）
- **工厂类**: `PreprocessingMethodFactory`
- **注册装饰器**: `@register_preprocessing("name")`
- **基类**: `BaseFeaturePreprocessing`
- **模板文件**: `habit/core/habitat_analysis/feature_preprocessing/custom_preprocessing_template.py`
- **说明**: 统一 DataFrame pipeline；群体级状态由 ``PreprocessingState`` 缓存
...
- **模板文件**: `habit/core/habitat_analysis/extractors/custom_feature_extractor_template.py`
...
- **用途**: 添加自定义的聚类算法
- **工厂函数**: `get_clustering_algorithm`
- **注册装饰器**: `@register_clustering("name")`
- **基类**: `BaseClustering`
- **模板文件**: `habit/core/habitat_analysis/algorithms/custom_clustering_template.py`
...
**4. 自定义habitat pipeline 模式**

- **用途**: 添加新的生境分割流程（例如新的step 组合，
- **注册位置**: `habit/core/habitat_analysis/habitat_analysis.py` 中的 `_PIPELINE_RECIPES`
- **核心接口**: recipe 函数返回 `(name, step)` 列表，交组`HabitatPipeline`
- **参考文档**: `docs/source/development/module_architecture.rst`
...
.. code-block:: python
   ...
   def _build_my_mode_steps(config, feature_service, clustering_service, habitat_image_writer):
       # Return ordered pipeline steps for the new mode.
       steps: list[tuple[str, BasePipelineStep]] = []
       return steps

   _PIPELINE_RECIPES["my_mode"] = _build_my_mode_steps
...
如果您的自定义组件对其他用户也有价值，可以考虑贡献分HABIT 项目。
...
- 查看 :doc:`../customization/index_zh` 了解详细的扩展指单
- 参见 :doc:`../user_guide/index_zh` 了解各个模块的使用方法
- 查看 :doc:`../tutorials/index` 学习完整的教程
