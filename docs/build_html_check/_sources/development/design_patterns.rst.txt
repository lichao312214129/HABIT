...
``config.HabitatSegmentation.clustering_mode`` 在
...
.. code-block:: python
   ...
   class HabitatAnalysis:
       def __init__(self, config, feature_service, clustering_service, habitat_image_writer, logger):
           self.config = config
           self.feature_service = feature_service
           self.clustering_service = clustering_service
           self.habitat_image_writer = habitat_image_writer
           self.logger = logger
           # HabitatResultPublisher is built inside __init__ (not injected) so CSV and
           # optional image publishing stay outside individual pipeline steps.
